    ...
    public partial class uc_Board_Specials : UserControl
    {
        ...
        public void Refresh()
        {
            ...
            if (ID_Joueur == 1)
            {
                ...
                {
                    NomImage = "img_Special_card_" + Tools.GetNextPlaceCard(CptCartes).ToString("D2");
                    NomLbl_Marks = "lbl_Special_Marks_" + Tools.GetNextPlaceCard(CptCartes).ToString("D2");
                    NomLbl_Background = "IsLinked_" + Tools.GetNextPlaceCard(CptCartes).ToString("D2");

                    //Mise à jour des éléments du UserControle
                    foreach (var grid in MainGrid_Specials.Children)
                    {
                        if (grid is Grid)
                        {
                            foreach (var tmp in (grid as Grid).Children)
                            {
                                ...
                                if (tmp is Label && (tmp as Label).Name == NomLbl_Marks)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        (tmp as Label).Content = "(" + Tools.CurrentGame.J1_Board_Specials[CptCartes].Marqueur.NbMarqueur + ") " + Tools.CurrentGame.J1_Board_Specials[CptCartes].Marqueur.MarqueurName;
                        ...
                        if (grid is Grid)
                        {
                            foreach (var tmp in (grid as Grid).Children)
                            {
                                ...
                                if (tmp is Label && (tmp as Label).Name == NomLbl_Marks)
                                {
                                    ...
                                    {
                                        (tmp as Label).Content = "(" + Tools.CurrentGame.J2_Board_Specials[CptCartes].Marqueur.NbMarqueur + ") " + Tools.CurrentGame.J2_Board_Specials[CptCartes].Marqueur.MarqueurName;
                                        (tmp as Label).Visibility = Visibility.Visible;
                                    }
                                }
                            }
        ...
        private void Show_Card(object sender, System.Windows.Input.MouseEventArgs e)
        {
            ...
            NomImage_Carte = "img_Special_card_" + indice_card.ToString("D2");

            foreach (var grd in MainGrid_Specials.Children)
            {
                if (grd is Grid && (grd as Grid).Name == NomGrid_Carte)
                {
                    ...
                    {
                        foreach (var img_view in MainGrid_Specials.Children)
                        {
                    ...
                    if (grd is Grid && (grd as Grid).Name == NomGrid_Carte)
                    {
                        foreach (var img_card in (grd as Grid).Children)
                        {
                            if (img_card is Image && (img_card as Image).Name == NomImage_Carte && (img_card as Image).Source != new BitmapImage())
                            {
                                ...
                                Tools.CurrentGame.J1_Deck.Cartes.Add(Tools.CurrentGame.J1_Board_Specials[Tools.GetIndiceCardFromBoard(indice_card)].ToCard());
                                Tools.CurrentGame.J1_Board_Specials.Remove(Tools.CurrentGame.J1_Board_Specials[Tools.GetIndiceCardFromBoard(indice_card)]);
                                Refresh();
            ...
            if (ID_Joueur == 1)
            {
                ...
                foreach (var grd in MainGrid_Specials.Children)
                {
                    if (grd is Grid && (grd as Grid).Name == NomGrid_Carte)
                    {
                        ...
                        {
                            (img_card as Image).Source = new BitmapImage();
                            Tools.CurrentGame.J1_Cimetiere.Add(Tools.CurrentGame.J1_Board_Specials[Tools.GetIndiceCardFromBoard(indice_card)]);
                            Tools.CurrentGame.J1_Board_Specials.Remove(Tools.CurrentGame.J1_Board_Specials[Tools.GetIndiceCardFromBoard(indice_card)]);
                            Refresh();
                            Tools.CurrentMainWindow.uc_DeckLists_Refresh(ID_Joueur);
                            break;
                ...
                foreach (var grd in MainGrid_Specials.Children)
                {
                    if (grd is Grid && (grd as Grid).Name == NomGrid_Carte)
                    {
                        foreach (var img_card in (grd as Grid).Children)
                        {
                            ...
                            {
                                (img_card as Image).Source = new BitmapImage();
                                Tools.CurrentGame.J1_Exil.Add(Tools.CurrentGame.J1_Board_Specials[Tools.GetIndiceCardFromBoard(indice_card)]);
                                Tools.CurrentGame.J1_Board_Specials.Remove(Tools.CurrentGame.J1_Board_Specials[Tools.GetIndiceCardFromBoard(indice_card)]);
                                Refresh();
                                Tools.CurrentMainWindow.uc_DeckLists_Refresh(ID_Joueur);
                                break;
